...
* ``Probabilité box Suprême`` : 3%
* ``Durabilitée`` : 4 jours à partir du moment où vous l'utilisez.

> Si vous utilisez ce parchemin alors que vous avez déjà un grade supérieur à Titan, celui si sera supprimé.

### {{image#items/others/parchment}} Parchemin Légende
 
* ``Fonctionnement`` : Lorsque vous utilisez ce parchemin, vous recevez directement le grade Légende et donc tous les avantages qui sont liés avec ce grade [ #commandes/grade].
* ``Probabilité box Suprême`` : 2%
* ``Durabilitée`` : 4 jours à partir du moment où vous l'utilisez.

> Si vous utilisez ce parchemin alors que vous avez déjà un grade supérieur à Légende, celui si sera supprimé. 
